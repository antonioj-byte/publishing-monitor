...
def _diagnose_country(report_filter: ReportFilter) -> None:
    ...
    if total_geo == 0:
        ...
        wider = report_filter.days or 7
        if wider >= 14:
            hint = f"/informe 30 {report_filter.location_label.lower()}"
        elif wider >= 7:
            hint = f"/informe 14 {report_filter.location_label.lower()}"
        else:
            hint = f"/informe 7 {report_filter.location_label.lower()}"
        print("⚠️  Nada en la ventana. Prueba:")
        print(f"    {hint}")
...
def main() -> None:
    ...
    print(f"API key configurada:   {'sí' if api_ok else 'NO — clasificación offline'}")
    print()
    ...
    if not api_ok:
        ...
        print(f"⚠️  Sin {key_name} el agente de clasificación usa modo offline (score 3 genérico).")
